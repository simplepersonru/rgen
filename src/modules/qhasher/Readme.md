...
**QHasher** — модуль rgen для автоматической генерации функции qHash() для структур и классов C++

### Параметры и атрибуты:
| Аннотация | Описание |
| --- | --- |
| `@QHasher` | Помечает структуру/класс для генерации |
...
struct TestStruct
{
    RGEN_DERIVE(friend, TestStruct, QHasher)

    int id; ///< @Ignore
    QString name;
};

TEST(QHasher, TestStruct)
{
...
- [`derive_test.rgen.cxx`](https://gitea.redkit-lab.work/redkit-lab/redkit-gen/src/branch/master/src/tests/doc_tests/test/derive_test.rgen.cxx) <br>
